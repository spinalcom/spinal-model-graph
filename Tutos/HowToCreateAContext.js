...
const spinalContext = new SpinalGraphLib.SpinalContext(contextType, contextName,
  element);
...
childPromise.then(child => {
  ...
  if (!child.info.hasOwnProperty("name") || child.info.name.get() !==
    contextName) {
    console.error(
      "hum something is wrong please notify this problem to the maintainer of the graph."
    );
  } else {
