...
describe("How to use map", function() {
  describe("Error handling", function() {
    ...
    it("should throw an error if the callback function is missing",
      async function() {
        let error = false;
    ...
    it(
      "should throw an error if the callback function is not a function",
      async function() {
        let error = false;
  ...
  describe("Basic callback manipulation", function() {
    ...
    it("should return second gen and undefined for other nodes nodes",
      async function() {
        ...
        const secondGen1 = new lib.SpinalNode(undefined,
          "secondGen");
        const secondGen2 = new lib.SpinalNode(undefined,
          "secondGen");
        const thirdGen1 = new lib.SpinalNode(undefined, "thirdGen");

        await Promise.all([
          root.addChild(firstGen1, DEFAULT_RELATION_NAME,
            DEFAULT_RELATION_TYPE),
          root.addChild(firstGen2, DEFAULT_RELATION_NAME,
            DEFAULT_RELATION_TYPE),
          root.addChild(firstGen3, DEFAULT_RELATION_NAME,
            DEFAULT_RELATION_TYPE),
          firstGen2.addChild(secondGen1, DEFAULT_RELATION_NAME,
            DEFAULT_RELATION_TYPE),
          firstGen3.addChild(secondGen2, DEFAULT_RELATION_NAME,
            DEFAULT_RELATION_TYPE),
          secondGen2.addChild(thirdGen1, DEFAULT_RELATION_NAME,
            DEFAULT_RELATION_TYPE)
        ]);
        ...
        assert.deepStrictEqual(secondGen, [
          ...
          secondGen1, secondGen2, undefined
        ]);
      });
  });
});
...
describe("How to use mapInContext", function() {
  describe("Error handling", function() {
    it(
      "should throw an error if the starting node, the context or the callback is missing",
      async function() {
    ...
    it("should throw an error if the context is not a SpinalContext",
      async function() {
        ...
        error = false;
        await mapInContext(DEFAULT_NODE, DEFAULT_CONTEXT,
          DEFAULT_FUN).catch(() => {
          error = true;
        });
        assert(!error);
    ...
    it("should not fall in infinite loops", async function() {
      ...
      const foundChild = await mapInContext(node1, context,
        DEFAULT_FUN);
  ...
  describe("How to use context", function() {
    it(
      "should mapInContext all the nodes from the given relation names",
      async function() {
        ...
        let foundChildren = await mapInContext(parent, context2,
          DEFAULT_FUN);
        ...
        foundChildren = await mapInContext(parent, context1,
          DEFAULT_FUN);
