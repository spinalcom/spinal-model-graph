...
describe("How to use forEachInContext", function() {
  describe("Error handling", function() {
    it(
      ...
      async function() {
        ...
        error = false;
        await forEachInContext(DEFAULT_NODE, undefined, DEFAULT_FUN)
          .catch(() => {
            error = true;
          });
        assert(error);
    ...
    it("should throw an error if the context is not a SpinalContext",
      async function() {
        ...
        error = false;
        await forEachInContext(DEFAULT_NODE, DEFAULT_CONTEXT,
          DEFAULT_FUN).catch(() => {
          error = true;
        });
        assert(!error);
  ...
  describe("How to use context", function() {
    it(
      "should forEachInContext all the nodes from the given relation names",
      async function() {
        ...
        await forEachInContext(parent, context2, node =>
        ...
        foundChildren = [];
        await forEachInContext(parent, context1, node =>
          foundChildren.push(node));
