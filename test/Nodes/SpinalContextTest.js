...
describe("SpinalContext", function() {
  describe("How to use the constructor", function() {
    it("should create a context with default values", async function() {
      ...
      assert.strictEqual(context.getName().get(),
        DEFAULT_SPINAL_CONTEXT_NAME);
      assert.strictEqual(context.getType().get(),
        DEFAULT_SPINAL_CONTEXT_TYPE);
  ...
  describe("How to get information about the context", function() {
    describe("How to use getRelationNames", function() {
      it(
        "should return all the relation names that the context knows",
        function() {
          let context = new lib.SpinalContext();
          ...
          assert.strictEqual(relationNames[0].get(), relation.getName()
            .get());
        });
  ...
  describe("How to register relations", function() {
    describe("How to use addRelationNames", function() {
      it("should add a relation name to the list", function() {
        ...
        const res = context.addRelationNames(
          DEFAULT_RELATION_NAME);
        ...
        assert.strictEqual(relationNames.length, 1);
        assert.strictEqual(relationNames[0].get(),
          DEFAULT_RELATION_NAME);
      });

      it("should not add the known relation name to the list",
        function() {
          let context = new lib.SpinalContext();
      ...
      it("should not add the known relation names to the list",
        function() {
          let context = new lib.SpinalContext();
          const name1 = DEFAULT_RELATION_NAME + "1";
          const name2 = DEFAULT_RELATION_NAME + "2";
  ...
  describe("How to add children to the context", function() {
    describe("How to use addChild", function() {
      it(
        "should add a child to the context with a SPINAL_RELATION_PTR_LST_TYPE type",
        async function() {
          let context = new lib.SpinalContext();

          await context.addChild(DEFAULT_NODE,
            DEFAULT_RELATION_NAME);
          assert(context.hasRelation(DEFAULT_RELATION_NAME, lib
    ...
    describe("How to use addChildInContext", function() {
      it(
        ...
        async function() {
          ...
          await context.addChildInContext(DEFAULT_NODE,
            DEFAULT_RELATION_NAME);
          assert(context.hasRelation(DEFAULT_RELATION_NAME, lib
            .SPINAL_RELATION_PTR_LST_TYPE));
        });

      it("should overload SpinalNode.addChildInContext correctly",
        async function() {
          let context = new lib.SpinalContext();
      ...
      it(
        "should be associated to the node (has a context) by default",
        async function() {
          let context = new lib.SpinalContext();
          let node = new lib.SpinalNode();

          await context.addChildInContext(node,
            DEFAULT_RELATION_NAME);

          assert.deepStrictEqual(node.getContextIds(), [context
            .getId().get()
          ]);
        });
    });
