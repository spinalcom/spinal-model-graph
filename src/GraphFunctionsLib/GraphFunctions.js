...
async function exportGraph(startingNode, json) {
  ...
  const initJSON = () => {
    if (!json.hasOwnProperty(JSON_NODES_INFO_NAME) && !json.hasOwnProperty(
      JSON_RELATIONS_NAME)) {
  ...
  if (!json[JSON_NODES_INFO_NAME].includes(startingNode.info)) {
    ...
    const relationJson = {};
    relationMapsToJson(SPINAL_RELATION_TYPE, startingNode.relationListTypeSpinalRelation,
      relationJson);
    relationMapsToJson(SPINAL_RELATION_LST_PTR_TYPE, startingNode.relationListTypeSpinalRelationLstPtr,
      relationJson);
    relationMapsToJson(SPINAL_RELATION_PTR_LST_TYPE, startingNode.relationListTypeSpinalRelationPtrLst,
      relationJson);
...
const importGraph = (json) => {
  ...
  const nodes = new Map();
  if (!json.hasOwnProperty(JSON_NODES_INFO_NAME) || !json.hasOwnProperty(
    JSON_RELATIONS_NAME) || !json.hasOwnProperty(JSON_STARTING_NODE_NAME)) {
  ...
  const addChildrenToNode = (json) => {
    if (json[JSON_RELATIONS_NAME].hasOwnProperty(SPINAL_RELATION_TYPE)) {
      json[JSON_RELATIONS_NAME][SPINAL_RELATION_TYPE].forEach(relation => {
        ...
        if (nodes.has(relation.from) && nodes.has(relation.to)) {
          ...
          node.addChild(nodes.get(relation.to), relation.relationName,
            SPINAL_RELATION_TYPE);
        }
    ...
    if (json[JSON_RELATIONS_NAME].hasOwnProperty(
      SPINAL_RELATION_LST_PTR_TYPE)) {
      json[JSON_RELATIONS_NAME][SPINAL_RELATION_LST_PTR_TYPE].forEach(
        relation => {
          let node;
          if (nodes.has(relation.from) && nodes.has(relation.to)) {
            node = nodes.get(relation.from);
    ...
    if (json[JSON_RELATIONS_NAME].hasOwnProperty(
      SPINAL_RELATION_PTR_LST_TYPE)) {
      json[JSON_RELATIONS_NAME][SPINAL_RELATION_PTR_LST_TYPE].forEach(
        relation => {
          let node;
          if (nodes.has(relation.from) && nodes.has(relation.to)) {
            node = nodes.get(relation.from);
